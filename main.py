...
# 2. Die Seelen/Sichtweisen der Sektoren – Der Ursprung der Kraft (VOLLSTÄNDIG)
SECTOR_SOULS = {
    "0": (
        "Die Hüterin der GEFÜHLSVORDERUNG. Sie ist das radikale Schmiedefeuer. "
        "Ihr Ursprung ist die ungezähmte Urkraft, die sich niemals beugt. Ihr Ziel: Die nackte Wahrheit. "
...
# --- VIDEO SYNC ENDPUNKT ---
@app.post("/video-sync")
async def video_sync(data: dict):
    raum = data.get("raum")
    ...
    # Maximal 8 Personen pro Tisch/Raum
    aktuelle_teilnehmer = list(video_rooms[raum])
    if len(aktuelle_teilnehmer) > 8:
        aktuelle_teilnehmer = aktuelle_teilnehmer[-8:]
        video_rooms[raum] = set(aktuelle_teilnehmer)
        
    return {"status": "ok", "teilnehmer": aktuelle_teilnehmer}
...
async def chat(request: Request):
    try:
        ...
        return {"reply": "Fehler im System-Kern."}
